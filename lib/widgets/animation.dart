    ...
    addStatusListener((status) {
      if (status == AnimationStatus.completed) {
        ...
        if (int.parse(inhaleHoldController.text) > 0) {
          BreathingAnimation.isHold = true; // Set inhale hold state if specified
        }
        ...
        if (int.parse(exhaleHoldController.text) > 0) {
          BreathingAnimation.isHold = true; // Set exhale hold state if specified
        }
        // Delayed logic after exhale phase (increment count, set inhale to true, hold to false, and forward the animation)
        Future.delayed(Duration(seconds: int.parse(exhaleHoldController.text)), () {
...
class BreathingAnimation {
  ...
  static TextStyle smallTextStyle = const TextStyle(fontSize: 60.0); // Starting text style
  ...
  static void runAnimation(int times) {
    ...
    textController.repeatEx(times: times); // Repeat the animation based on the specified times
  }
